#include "had_and_quant_gs.h"
#include <cuda_fp16.h>

// ==================== Hadamard Transform + Quantization Fused Kernel (Group Size Flexible) ====================
...
// Helper to select kLogThreadsPerRow based on GROUP_SIZE
// GROUP_SIZE 128: 32 threads -> kLogThreadsPerRow = 5
// GROUP_SIZE 64:  16 threads -> kLogThreadsPerRow = 4
// GROUP_SIZE 32:  8 threads  -> kLogThreadsPerRow = 3
...
struct ThreadsPerRowHelper<128> {
    ...
    static constexpr int kLogThreadsPerRow = 5;
};

template<>
struct ThreadsPerRowHelper<64> {
    static constexpr int value = 16;
    static constexpr int kLogThreadsPerRow = 4;
};

template<>
struct ThreadsPerRowHelper<32> {
    static constexpr int value = 8;
    static constexpr int kLogThreadsPerRow = 3;
};

// Fused Hadamard Transform + Quantization Kernel (Template Version)
template<int GROUP_SIZE>
__global__ void had_and_quant_kernel_gs(
    ...
    constexpr int kLogThreadsPerRow = ThreadsPerRowHelper<GROUP_SIZE>::kLogThreadsPerRow;
    constexpr int kElemsPerThread = GROUP_SIZE / kNThreadsPerRow; // elements per thread
    ...
    hadamard_mult_thread<kElemsPerThread>(x_vals);
    
    // 3. Warp-level Hadamard transform (using shuffle)
    hadamard_mult_warp<kLogThreadsPerRow, kElemsPerThread>(x_vals, tid_in_warp);
    ...
    // Each thread handles kElemsPerThread elements -> produces kElemsPerThread/2 packed bytes
...
template __global__ void had_and_quant_kernel_gs<32>(
    const __half* __restrict__ data,
    Int4Storage* __restrict__ quantized_data,
    __half* __restrict__ scales,
    uint32_t M
...
template __global__ void had_and_quant_kernel_gs<64>(
    const __half* __restrict__ data,
    Int4Storage* __restrict__ quantized_data,
    __half* __restrict__ scales,
    uint32_t M
...
template __global__ void had_and_quant_kernel_gs<128>(
    const __half* __restrict__ data,
    Int4Storage* __restrict__ quantized_data,
    __half* __restrict__ scales,
    uint32_t M
...
void had_and_quant_host_impl(
    ...
    had_and_quant_kernel_gs<GROUP_SIZE><<<num_blocks, kThreadsPerBlock>>>(
        data,
        quantized_data,
        scales,
        M
...
template void had_and_quant_host_impl<32>(
    const half* data,
    Int4Storage* quantized_data,
    half* scales,
    uint32_t M
...
template void had_and_quant_host_impl<64>(
    const half* data,
    Int4Storage* quantized_data,
    half* scales,
    uint32_t M
...
template void had_and_quant_host_impl<128>(
    const half* data,
    Int4Storage* quantized_data,
    half* scales,
    uint32_t M
...
void had_and_quant_host_gs(
    ...
    switch (group_size) {
        case 32:
            had_and_quant_host_impl<32>(data, quantized_data, scales, M);
            break;
        case 64:
            had_and_quant_host_impl<64>(data, quantized_data, scales, M);
            break;
        case 128:
        default:
            had_and_quant_host_impl<128>(data, quantized_data, scales, M);
            break;
    }
}
