...
struct hadamard_kernel_traits {
    using input_t = input_t_;
    
    // 基本配置参数
    static constexpr int kNElts = kNElts_;  // 一个线程处理的元素数（默认16）
    ...
    // 导出参数
    static constexpr int kLogN = kLogN_;  // log2(128) = 7
    static constexpr int N = 1 << kLogN;  // 128 for log2(128) = 7
...
// ==================== 注意：当前配置下每行只需8个线程，全在同一warp内，无需跨warp交换 ====================

// ==================== 主Kernel ====================

template<typename Ktraits>
__global__ __launch_bounds__(Ktraits::kNThreads)
void hadamard_transform_kernel_optimized(
...
void launch_hadamard_kernel(torch::Tensor& data, int rows, int cols) {
    using Ktraits = hadamard_kernel_traits<kNElts, kNThreadsPerRow, kRowsPerWarp, kRowsPerBlock, kLogN, __half>;
    constexpr int kNThreads = Ktraits::kNThreads;
    ...
    auto kernel = &hadamard_transform_kernel_optimized<Ktraits>;
...
void had_trans_half_cuda(torch::Tensor& data) {
    ...
    if (data.dim() != 2) {
        throw std::runtime_error(
            "Error: Input tensor must be 2D. Please reshape to 2D tensor with last dimension = 128. "
    ...
    if (cols != 128) {
        throw std::runtime_error(
            "Error: The last dimension must be 128, but got " + std::to_string(cols)
        );
    }

    // 自适应配置策略：根据矩阵规模选择最优配置
    // 核心原则：
    // 1. 保持每个warp处理一行（32线程），确保内存合并访问
    // 2. 小矩阵：减少每block的行数，增加block并行度
    // 3. 大矩阵：增加每block的行数，减少调度开销
    
    constexpr int kNElts = 4;            // 每个线程处理4个元素
    constexpr int kNThreadsPerRow = 32;  // 32线程处理一行（1个完整warp）
    constexpr int kRowsPerWarp = 1;      // 每个warp处理1行
    constexpr int kLogN = 7;             // log2(128)
    
    // 根据行数选择每个block处理的行数
    if (rows <= 512) {
        // 极小规模：每block 2行 = 64线程，最大化block数量
        constexpr int kRowsPerBlock = 2;
        launch_hadamard_kernel<kNElts, kNThreadsPerRow, kRowsPerWarp, kRowsPerBlock, kLogN>(data, rows, cols);
    } else if (rows <= 1024) {
        // 小规模：每block 4行 = 128线程
        constexpr int kRowsPerBlock = 4;
        launch_hadamard_kernel<kNElts, kNThreadsPerRow, kRowsPerWarp, kRowsPerBlock, kLogN>(data, rows, cols);
    } else if (rows <= 2048) {
        // 中小规模：每block 8行 = 256线程
        constexpr int kRowsPerBlock = 8;
        launch_hadamard_kernel<kNElts, kNThreadsPerRow, kRowsPerWarp, kRowsPerBlock, kLogN>(data, rows, cols);
    } else if (rows <= 4096) {
        // 中等规模：每block 16行 = 512线程
        constexpr int kRowsPerBlock = 16;
        launch_hadamard_kernel<kNElts, kNThreadsPerRow, kRowsPerWarp, kRowsPerBlock, kLogN>(data, rows, cols);
    } else {
        // 大规模：每block 32行 = 1024线程，最大化每block的工作量
