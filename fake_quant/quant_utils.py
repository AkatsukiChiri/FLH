...
class ActQuantWrapper(torch.nn.Module):
    ...
    def extra_repr(self) -> str:
        ...
        str_ += f'\nQuantizer group_size: {self.group_size}'
        return str_
    ...
    def forward(self, x):
        ...
        if self.online_had:
            if self.fp32_had:
                x = hadamard_utils.matmul_had_cuda(x.float(), self.group_size).to(x_dtype)
            else:
                x = hadamard_utils.matmul_had_cuda(x, self.group_size)
...
class WeightQuantizer(torch.nn.Module):
    ...
    def configure(
        self,
        bits, perchannel=False, sym=True, groupsize=-1,
    ...
    def find_params_group(self, x):
        init_shape = x.shape
        reshaped_x = x.reshape(-1, x.shape[-2], x.shape[-1] // self.groupsize, self.groupsize)
        xmax = torch.amax(reshaped_x, dim=3, keepdim=True)
