...
__global__ void __launch_bounds__(256) flh_gemm_i4_dequant_o16_kernel(
  const uint8_t *A,
  const uint8_t *B,
  half *D,
  const int M_GLOBAL,
...
extern "C" void flhDenseLayerGEMM_i4_o16(
  ...
  cudaFuncSetAttribute(
    flh_gemm_i4_dequant_o16_kernel,
    cudaFuncAttributeMaxDynamicSharedMemorySize,
    SHMEM_SZ
  );
 
  flh_gemm_i4_dequant_o16_kernel<<<gridDim, blockDim, SHMEM_SZ>>>(
    A, B, D,
